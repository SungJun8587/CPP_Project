...
void ProducerThread()
{
	...
	while( !g_bStopProducerThread.load() )
	{
		...
		for( int i = 0; i < MAX_ROWS; i += BATCH_SIZE )
		{
			...
			PushDBAsyncRequest<CMySQLAsyncSrv, PRODUCER_DATA_BATCH_REQ>(MEMBER_DB_ASYNC, DBASYNC_BULKADD_PRODUCER_REQ, [i, currentBatchSize](PRODUCER_DATA_BATCH_REQ* pDBAsync) {
...
bool FetchMigrationData(int32 batchSize, std::unique_ptr<PRODUCER_DATA[]>& outBatchReq, int32& outFetchedCount)
{
	...
	// 6. MySQL 쿼리 실행 및 결과 집합(MYSQL_RES) 오픈
	...
	// 7. Resultset을 순회하면서 배치 버퍼에 데이터 패킹
	while( MYSQL_ROW Row = mysql_fetch_row(pRes) )
	{
		...
		int i = 0;
		// 7-1. Row로부터 컬럼 값 추출 및 구조체 필드 변환
...
void DBProducerThread()
{
	...
	while( !g_bStopProducerThread.load() )
	{
		...
		PushDBAsyncRequest<CMySQLAsyncSrv, CONSUMER_DATA_BATCH_REQ>(MEMBER_DB_ASYNC, DBASYNC_BULKADD_CONSUMER_REQ, [rawProducers, fetchedCount](CONSUMER_DATA_BATCH_REQ* pDBAsync) {
...
void DbAsyncAdd()
{
	for( int i = 1; i <= 20; i++ )
	{
		PushDBAsyncRequest<CMySQLAsyncSrv, PRODUCER_DATA_BATCH_REQ>(MEMBER_DB_ASYNC, DBASYNC_ADD_PRODUCER_REQ, [c_i = i](PRODUCER_DATA_BATCH_REQ* pDBAsync) {
...
int main()
{
	...
	// 3. C API MySQL 라이브러리 초기화 (프로그램 전체에서 단 한 번 호출)
	...
	// 10. 메인 모니터링 루프 (완료 상태 감시 및 키보드 ESC 입력 대기)
	while( 1 )
	{
	...
	// 14. MySQL 라이브러리 메모리 정리 (프로그램 종료 시 한 번만 호출)
	mysql_library_end();
