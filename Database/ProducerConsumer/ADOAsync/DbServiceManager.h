...
private:
	...
	// std::unique_ptr<CAdoAsyncSrv> _gameDB;   // 게임 콘텐츠 DB 비동기 서비스
	// std::unique_ptr<CAdoAsyncSrv> _logDB;    // 로그 축적 DB 비동기 서비스
	//***************************************************************************
	std::unique_ptr<CAdoAsyncSrv> _memberDB;	// 멤버 도메인 전용 비동기 DB 서비스 객체
};
