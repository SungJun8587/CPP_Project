...
#define DBASYNC_LIST_ALLSETS_PRODUCER_REQ 4
#define DBASYNC_BULKADD_PRODUCER_REQ 5
typedef struct _PRODUCER_DATA_BATCH_REQ : public st_DBAsyncRq
{
	// 기본 1개 슬롯을 미리 확보해 둔다 — DBASYNC_ADD_PRODUCER_REQ/
	...
	std::vector<PRODUCER_DATA>	_producers;
	int				_dataCount;
	uint32			_lastNo;
...
typedef struct _CONSUMER_DATA_BATCH_REQ : public st_DBAsyncRq
{
	...
	std::vector<CONSUMER_DATA>	_consumers;
	int				_dataCount;
