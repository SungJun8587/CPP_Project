// ODBCAsync.cpp : 이 파일에는 'main' 함수가 포함됩니다. 거기서 프로그램 실행이 시작되고 종료됩니다.
//
...
COdbcConnPool* g_pProducerConnPool = nullptr;           // 생산자 전용 ODBC 커넥션풀
...
void ProducerThread()
{
	...
	while( !g_bStopProducerThread.load() )
	{
		...
		for( int i = 0; i < MAX_ROWS; i += BATCH_SIZE )
		{
			...
			PushDBAsyncRequest<COdbcAsyncSrv, PRODUCER_DATA_BATCH_REQ>(MEMBER_DB_ASYNC, DBASYNC_BULKADD_PRODUCER_REQ, [i, currentBatchSize](PRODUCER_DATA_BATCH_REQ* pDBAsync) {
...
bool FetchMigrationData(int32 batchSize, std::unique_ptr<PRODUCER_DATA[]>& outBatchReq, int32& outFetchedCount)
{
	...
	OdbcConnGuard pOdbcConn(g_pProducerConnPool);
	if( pOdbcConn == nullptr )
	{
		LOG_ERROR(_T("FetchMigrationData: No available ODBC connection in producer pool."));
		return false;
	}
	...
	// 4. Keyset Paging (No > currentLastNo) 기반 SQL 쿼리문 생성
	_tstring query = _T("SELECT No, Name1, Name2, Flag, Age FROM Producer WHERE No > ") +
		to_tstring(currentLastNo) + _T(" ORDER BY No ASC LIMIT ") + to_tstring(batchSize);

	// 5. ODBC Direct Query 실행
	if( !pOdbcConn->ExecDirect(query.c_str()) )
	{
		LOG_ERROR(_T("FetchMigrationData: Failed to execute query. Query: %s"), query.c_str());
		pOdbcConn->ClearStmt();
		return false;
	}

	// 6. 인출 데이터를 담을 동적 배열 버퍼 생성 및 메모리 초기화
	...
	int32 iName1Size = _countof(outBatchReq[0].tszName1);
	int32 iName2Size = _countof(outBatchReq[0].tszName2);

	// 7. 이전 컬럼 바인딩 해제 및 Bulk Fetch(AllSets) 바인딩 설정
	pOdbcConn->UnBindColStmt();
	pOdbcConn->AllSets(sizeof(PRODUCER_DATA), batchSize);

	pOdbcConn->BindCol(outBatchReq[0].nNo);
	pOdbcConn->BindCol(outBatchReq[0].tszName1, iName1Size);
	pOdbcConn->BindCol(outBatchReq[0].tszName2, iName2Size);
	pOdbcConn->BindCol(outBatchReq[0].bFlag);
	pOdbcConn->BindCol(outBatchReq[0].nAge);
	...
	// 10. 실제 인출 건수 계산 및 최대 PK 검증
	for( int i = 0; i < batchSize; ++i )
	{
		if( outBatchReq[i].nNo == 0 ) break;
		fetchedCount++;
		maxFetchedNo = outBatchReq[i].nNo;
	}
	...
	// 11. CAS(Compare-And-Swap) 연산을 통해 공유 PK 변수(g_sharedLastNo)를 최신값으로 원자적 갱신
...
void DBProducerThread()
{
	...
	while( !g_bStopProducerThread.load() )
	{
		...
		PushDBAsyncRequest<COdbcAsyncSrv, CONSUMER_DATA_BATCH_REQ>(MEMBER_DB_ASYNC, DBASYNC_BULKADD_CONSUMER_REQ, [rawProducers, fetchedCount](CONSUMER_DATA_BATCH_REQ* pDBAsync) {
...
//          람다 함수를 사용해 필드(번호, 이름, 바인딩 길이, 플래그, 나이)를 초기화한 뒤 
//          ODBC 비동기 DB 작업 큐(MEMBER_DB_ASYNC)에 추가합니다.
//***************************************************************************
void DbAsyncAdd()
{
	for( int i = 1; i <= 20; i++ )
	{
		PushDBAsyncRequest<COdbcAsyncSrv, PRODUCER_DATA_BATCH_REQ>(MEMBER_DB_ASYNC, DBASYNC_ADD_PRODUCER_REQ, [c_i = i](PRODUCER_DATA_BATCH_REQ* pDBAsync) {
...
void DbAsyncGet()
{
	PushDBAsyncRequest<COdbcAsyncSrv, PRODUCER_DATA_BATCH_REQ>(MEMBER_DB_ASYNC, DBASYNC_GET_PRODUCER_REQ, [](PRODUCER_DATA_BATCH_REQ* pDBAsync) {
		}, MAX_QUEUE_CAPACITY);
}
...
void DbAsyncList()
{
	PushDBAsyncRequest<COdbcAsyncSrv, PRODUCER_DATA_BATCH_REQ>(MEMBER_DB_ASYNC, DBASYNC_LIST_PRODUCER_REQ, [](PRODUCER_DATA_BATCH_REQ* pDBAsync) {
		}, MAX_QUEUE_CAPACITY);
}

//***************************************************************************
// @brief  전체 결과 집합 생산자 데이터 목록 조회 요청 등록
// @details 다중 결과 집합(Multiple Result Sets)을 포함한 생산자 목록 조회를 위한 
//          비동기 DB 요청(DBASYNC_LIST_ALLSETS_PRODUCER_REQ)을 작업 큐에 등록합니다.
//***************************************************************************
void DbAsyncListAllsets()
{
	PushDBAsyncRequest<COdbcAsyncSrv, PRODUCER_DATA_BATCH_REQ>(MEMBER_DB_ASYNC, DBASYNC_LIST_ALLSETS_PRODUCER_REQ, [](PRODUCER_DATA_BATCH_REQ* pDBAsync) {
		}, MAX_QUEUE_CAPACITY);
}
...
// 그래야 COdbcConnPool의 헬스체크/재연결 워커 스레드가 gpMemory/gpThreadManager가
...
int main()
{
	// 특정 할당 번호 추적용 (필요 시 주석 해제)
	// _CrtSetBreakAlloc(2866);
	...
	if( producerThreadCnt > 0 )
	{
		...
		g_pProducerConnPool = new COdbcConnPool(producerThreadCnt);
		COdbcConnPool::TReconnectConfig reconnectCfg;
		reconnectCfg.nWorkerCount = producerThreadCnt;

		if( !g_pProducerConnPool->Init(dbNode._dbClass, dbNode._tszDSN, reconnectCfg) )
		{
