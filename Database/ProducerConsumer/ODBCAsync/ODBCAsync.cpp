...
// 생산자 DB 커넥션풀 선언
COdbcConnPool* g_pProducerConnPool = nullptr;
...
bool FetchMigrationData(int32 batchSize, std::unique_ptr<PRODUCER_DATA[]>& outBatchReq, int32& outFetchedCount)
{
	...
	// 2. AllSets 설정: 행 바인딩 단위는 개별 데이터 구조체 크기, 
	// SQL_ATTR_ROW_ARRAY_SIZE는 이번에 가져올 batchSize로 설정
	pOdbcConn->AllSets(sizeof(PRODUCER_DATA), batchSize);

	// 3. 첫 번째 행의 컬럼 주소들을 정확히 바인딩
...
int main()
{
	...
	*/
	...
	/*
	PushAsyncRequest<PRODUCER_DATA_BATCH_REQ>(DBASYNC_LIST_ALLSETS_PRODUCER_REQ, [](PRODUCER_DATA_BATCH_REQ* pDBAsync) {
		});
	*/
	...
	if( false == COdbcAsyncSrv::Instance()->StartService(SERVER_CONFIG->GetDBNodeVec(), consumerThreadCnt) )	// DB Async Call Service 시작
	{
