...
// @brief 멤버/게임/로그 등 도메인별 COdbcAsyncSrv 인스턴스를 소유하고 이름 있는 접근자로 노출하는 프로세스 전역 매니저.
// @details
// [초기화 순서] 이 매니저 자체는 "최초 사용 시점 생성"(Instance() 최초 호출 시 생성)이라 다른 정적 초기화식과의 순서 경쟁에서 자유롭다.
// 다만 각 COdbcAsyncSrv 인스턴스는 생성만 될 뿐 StartService()가 자동으로 불리지는 않는다.
...
public:
	...
	// @return COdbcAsyncSrv& 멤버 DB 서비스 인스턴스 참조
	//***************************************************************************
	COdbcAsyncSrv& MemberDB() { return *_memberDB; }
...
private:
	...
	//***************************************************************************
	CDbServiceManager()
		: _memberDB(std::make_unique<COdbcAsyncSrv>())
	{
	}
	...
	// std::unique_ptr<COdbcAsyncSrv> _gameDB;   // 게임 콘텐츠 DB 비동기 서비스
	// std::unique_ptr<COdbcAsyncSrv> _logDB;    // 로그 축적 DB 비동기 서비스
	//***************************************************************************
	std::unique_ptr<COdbcAsyncSrv> _memberDB;	// 멤버 도메인 전용 비동기 DB 서비스 객체
};
