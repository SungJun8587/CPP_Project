...
public:
	...
	//***************************************************************************
	// @brief 파일 서버 업로드용 임시 토큰 발급을 요청합니다. 바디 없음.
	// @details [설계] 실제 파일 업로드(발급받은 토큰+파일서버주소로 HTTP
	//          POST)는 이 콘솔 클라이언트 범위 밖이다 — 여기서는 토큰
	//          발급까지의 프로토콜만 다루고, 발급된 토큰/파일서버 주소를
	//          화면에 보여주는 데서 그친다(ChatClient.cpp::HandleUploadTokenCmd 참고).
	//***************************************************************************
	void	SendRequestUploadTokenReq();

	//***************************************************************************
	// @brief 내 프로필 이미지 갤러리(업로드해둔 전체 목록) 조회를 요청합니다. 바디 없음.
	//***************************************************************************
	void	SendListProfileImagesReq();

	//***************************************************************************
	// @brief 갤러리에 있는 이미지 중 하나를 대표 프로필 이미지로 지정 요청합니다.
	//***************************************************************************
	void	SendSelectProfileImageReq(int64 imageId);

	//***************************************************************************
	// @brief 갤러리에서 이미지 하나를 삭제 요청합니다.
	//***************************************************************************
	void	SendDeleteProfileImageReq(int64 imageId);
